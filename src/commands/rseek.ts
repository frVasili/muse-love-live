...
export default class implements Command {
  public readonly slashCommand = new SlashCommandBuilder()
    .setName('rseek')
    .setDescription('seek backward in the current song')
  ...
  public async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (seekTime > player.getPosition()) {
      throw new Error('can\'t seek before the beginning of the song');
    }

    await Promise.all([
      player.seek(player.getPosition() - seekTime),
